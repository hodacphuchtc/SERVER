...
begin
  ...
  with danh_gia as (
    select h.id                                                                          as id,
           ...
           round(extract(epoch from (now() - coalesce(h.lan_day_du_lieu_cuoi, h.tao_luc))) / 60.0, 1) as phut,
           extract(epoch from (now() - coalesce(h.lan_day_du_lieu_cuoi, h.tao_luc))) / 60.0 > p_im_lang_phut as qua_han
    from public.hosts h
  ...
  mo as (
    ...
    select d.id, 'mat_lien_lac', 'nghiem_trong', d.phut, p_im_lang_phut
    from danh_gia d
    where d.qua_han
...
comment on function public.soat_mat_lien_lac(int) is
